...
const functions={
  ...
  requestHttpMethods(url,data,HttpMethod='post',isloading=false,commonParam=true,config=AxiosConfig){
    if(isloading)vantTool.showloading('正在加载中...');
    ...
    if(commonParam){
      ...
      if(store.state.openId){
        ...
        openid=functions.randomChar();
        data.logisticsInterface.openId=openid;
      }
      var dataDigest='';
      var commonParams;
      if(openid){
        args=Api.fromCode+openid+Api.apisecret;
        dataDigest=functions.createdMd5(args);
      }
      commonParams={
        dataDigest:dataDigest,
        fromCode:Api.fromCode,
      }
    }
